# MySQL 实操环境准备

> 5分钟快速搭建MySQL学习环境，然后开始学习！
>
> **本教程使用Docker方式，简单、干净**

---

## 快速开始（3步搞定）

### 步骤1：启动MySQL容器

```bash
docker run --name mysql-study \
  -e MYSQL_ROOT_PASSWORD=123456 \
  -p 3306:3306 \
  -v ~/mysql-study/data:/var/lib/mysql \
  -d mysql:8.0 \
  --character-set-server=utf8mb4 \
  --collation-server=utf8mb4_unicode_ci \
  --log-bin=mysql-bin \
  --binlog-format=ROW \
  --server-id=1
```

等待10秒让MySQL启动完成。

### 步骤2：连接MySQL

```bash
docker exec -it mysql-study mysql -uroot -p123456
```

### 步骤3：创建学习数据库

在MySQL中执行：

```sql
-- 创建学习数据库
CREATE DATABASE mysql_study DEFAULT CHARACTER SET utf8mb4;

-- 切换到学习数据库
USE mysql_study;

-- 创建测试表
CREATE TABLE users (
    id INT PRIMARY KEY AUTO_INCREMENT,
    name VARCHAR(50) NOT NULL,
    age INT,
    created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP
) ENGINE=InnoDB;

-- 插入测试数据
INSERT INTO users (name, age) VALUES
('张三', 25),
('李四', 30),
('王五', 28);

-- 验证
SELECT * FROM users;
```

看到3条数据？**环境搭建完成！** 现在可以开始学习 `01-架构与日志系统.md` 了！

---

## 常用命令

```bash
# 启动容器
docker start mysql-study

# 停止容器
docker stop mysql-study

# 重启容器
docker restart mysql-study

# 连接MySQL
docker exec -it mysql-study mysql -uroot -p123456

# 查看容器状态
docker ps -a | grep mysql-study

# 查看日志
docker logs mysql-study
```

---

## 验证环境（可选）

如果想验证binlog等配置是否生效：

```sql
-- 验证binlog是否开启
SHOW VARIABLES LIKE 'log_bin';
-- 应该显示 ON

-- 验证binlog格式
SHOW VARIABLES LIKE 'binlog_format';
-- 应该显示 ROW

-- 查看MySQL版本
SELECT VERSION();

-- 查看字符集
SHOW VARIABLES LIKE 'character%';
```

---

## 环境说明

- **数据存储**：`~/mysql-study/data/`（在你的Mac上）
- **root密码**：`123456`
- **端口**：`3306`
- **MySQL版本**：8.0
- **字符集**：utf8mb4

---

## 遇到问题？

### 容器无法启动

```bash
# 查看错误日志
docker logs mysql-study

# 端口被占用？改用其他端口
docker run --name mysql-study \
  -e MYSQL_ROOT_PASSWORD=123456 \
  -p 3307:3306 \
  -v ~/mysql-study/data:/var/lib/mysql \
  -d mysql:8.0 \
  --character-set-server=utf8mb4
```

### 连接被拒绝

```bash
# 检查容器是否运行
docker ps | grep mysql-study

# 如果没运行，启动它
docker start mysql-study
```

### 重新开始

```bash
# 删除容器和数据，重新来过
docker rm -f mysql-study
rm -rf ~/mysql-study/data
# 然后重新执行步骤1
```

---

## 附录：高级配置（按需查看）

<details>
<summary>点击展开：完整配置文件方式</summary>

如果需要更细粒度的配置控制，可以使用配置文件方式：

```bash
# 创建目录结构
mkdir -p ~/mysql-study/{conf,data,logs}

# 创建配置文件
cat > ~/mysql-study/conf/my.cnf << 'EOF'
[mysqld]
character-set-server=utf8mb4
collation-server=utf8mb4_unicode_ci
default-time-zone='+8:00'
server-id=1

# binlog配置
log-bin=mysql-bin
binlog_format=ROW
sync_binlog=1

# redo log配置
innodb_flush_log_at_trx_commit=1

# 慢查询日志
slow_query_log=1
slow_query_log_file=/var/log/mysql/mysql-slow.log
long_query_time=2

# 性能配置
innodb_buffer_pool_size=1G
max_connections=200

[mysql]
default-character-set=utf8mb4

[client]
default-character-set=utf8mb4
EOF

# 启动容器（挂载配置文件）
docker run --name mysql-study \
  -e MYSQL_ROOT_PASSWORD=123456 \
  -p 3306:3306 \
  -v ~/mysql-study/conf/my.cnf:/etc/mysql/conf.d/my.cnf \
  -v ~/mysql-study/data:/var/lib/mysql \
  -v ~/mysql-study/logs:/var/log/mysql \
  -d mysql:8.0
```

</details>

<details>
<summary>点击展开：数据备份与恢复</summary>

### 备份数据库

```bash
# 备份单个数据库
docker exec -e MYSQL_PWD=123456 mysql-study mysqldump -uroot \
  --single-transaction \
  mysql_study > ~/mysql-study-backup.sql

# 备份所有数据库
docker exec -e MYSQL_PWD=123456 mysql-study mysqldump -uroot \
  --all-databases \
  --single-transaction \
  > ~/mysql-all-backup.sql
```

### 恢复数据库

```bash
# 恢复数据库
docker exec -i -e MYSQL_PWD=123456 mysql-study mysql -uroot mysql_study \
  < ~/mysql-study-backup.sql
```

</details>

<details>
<summary>点击展开：查看binlog</summary>

```bash
# 查看binlog列表
docker exec -e MYSQL_PWD=123456 mysql-study mysql -uroot \
  -e "SHOW BINARY LOGS;"

# 查看binlog内容
docker exec -e MYSQL_PWD=123456 mysql-study mysql -uroot \
  -e "SHOW BINLOG EVENTS IN 'mysql-bin.000001' LIMIT 10;"
```

</details>

<details>
<summary>点击展开：性能监控命令</summary>

```sql
-- 查看当前连接
SHOW FULL PROCESSLIST;

-- 查看InnoDB状态
SHOW ENGINE INNODB STATUS\G

-- 查看慢查询数量
SHOW GLOBAL STATUS LIKE 'Slow_queries';

-- 查看buffer pool使用情况
SHOW STATUS LIKE 'Innodb_buffer_pool%';
```

</details>

---

**准备完成！开始学习 → `01-表设计基础与规范.md`** 🚀
